...
<script>
	function del_check() {
		...
		if($.trim($("#del_cont").val()).length == 0) {
			$('.delcont_error').html("<strong style='color:red; font-size:8px;'>"
			+"탈퇴 사유를 입력하세요!</strong>");
			$('#del_cont').val('').focus();
			return false;
		}
	...
	<form method="post" action="member_del_ok" onsubmit="return del_check();">
		<table id="mDel_t">
			...
			<tr>
				<th>비밀번호</th>
				<td>
					<input tpye="password" name="del_pwd" id="del_pwd" size="14">
				</td>
			</tr>
			<tr>
				<th>탈퇴사유</th>
				<td>
					...
					<br>
					<span class="delcont_error"></span><%-- 여기에 에러 메세지가 출력 --%>
				</td>
			</tr>
		</table>
