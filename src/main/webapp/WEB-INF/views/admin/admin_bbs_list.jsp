...
<div id="aMain_cont">
	<form method="get" action="admin_bbs_list">
					...
					<c:forEach var="b" items="${blist}">
						<tr>
							...
							<td>
								...
								<a href="admin_bbs_cont?no=${b.bbs_no}&page=${page}&state=cont">
							...
							<td align="center">
								<input type="button" value="수정" 
									onclick="location='admin_bbs_cont?no=${b.bbs_no}&page=${page}&state=edit';" />
								<input type="button" value="삭제"
									onclick="if(confirm('정말로 삭제할까요?') == true){
										location= 'admin_bbs_del?no=${b.bbs_no}&page=${page}';   
										}else{ return ;}" />
		...
		<div id="bList_menu">
			<input type="button" value="글쓰기" onclick="location='admin_bbs_write?page=${page}';" />
			<c:if test="${(!empty find_field) && (!empty find_name)}">
		...
		<select name="find_field">
			...
			<option value="bbs_cont"
				<c:if test="${find_field=='bbs_cont'}">${'selected'}</c:if>>
					내용
			</option>
		</select>
